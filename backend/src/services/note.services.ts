import { noteRepositories } from '../repositories/note.repositories';
...
class NoteServices {
    async getAllNotes(): Promise<QueryResult> {
        try {
            return await noteRepositories.getNotesList();
        } catch (error) {
    ...
    async getOne(id: NotesTypes.NoteID): Promise<QueryResult | null> {
        if (!id) {
            throw new APIError(NOTES_ERROR_MESSAGE.ID_REQUIRED_ERROR, RESPONSE_STATUS_CODE.BAD_REQUEST);
        }
        try {
            return await noteRepositories.getNoteById(id);
        } catch (error) {
    ...
    async createNote(title: NotesTypes.NoteTitle, content: NotesTypes.NoteContent): Promise<QueryResult> {
        ...
        try {
            return await noteRepositories.createNote(title, content);
        } catch (error) {
    ...
    async deleteNote(id: NotesTypes.NoteID): Promise<QueryResult> {
        if (!id) {
            throw new APIError(NOTES_ERROR_MESSAGE.ID_REQUIRED_ERROR, RESPONSE_STATUS_CODE.BAD_REQUEST);
        }
        ...
        try {
            return await noteRepositories.deleteNote(id);
        } catch (error) {
    ...
    async updateNote(id: NotesTypes.NoteID, title: NotesTypes.NoteTitle, content: NotesTypes.NoteContent): Promise<QueryResult> {
        ...
        try {
            return await noteRepositories.updateNote(id, title, content);
        } catch (error) {
